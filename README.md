...
Nodara supports [BOT Chain](https://www.botchain.ai) as a third EVM network alongside Ethereum and Base — added without touching the workflow engine, trigger/condition/action handlers, or the KeeperHub execution boundary, all of which were already chain-agnostic (every read/write already threads a `chainId` through instead of assuming one).

**Nodara is mainnet-only.** No testnet chain (Sepolia, Base Sepolia, or BOT Chain Testnet) is configured or selectable anywhere in the app — every supported network below is a production chain with real assets.

| Network | Chain ID | Native currency | RPC | Explorer |
|---|---|---|---|---|
| Ethereum | `1` | ETH | (see `RPC_URL_MAINNET`) | https://etherscan.io |
| Base | `8453` | ETH | (see `RPC_URL_BASE`) | https://basescan.org |
| BOT Chain | `677` | BOT | `https://rpc.botchain.ai` | https://scan.botchain.ai |

**Verification status:** BOT Chain's mainnet chain ID (677) was independently corroborated against [ChainList](https://chainlist.org/chain/677) (BOT native currency, `scan.botchain.ai` explorer — matches). `scan.botchain.ai` was confirmed to run [Blockscout](https://www.blockscout.com) (via its own footer attribution) — relevant because Blockscout's contract-verification flow differs from Etherscan-family explorers; see [contracts/README.md](contracts/README.md) for the exact verifier command.

### What changed for chain support

- `packages/shared/src/enums.ts` — added `BOT_CHAIN` (677) to the `ChainId` enum, the single source of truth consumed by both apps.
- `packages/blockchain/src/chains.ts` — BOT Chain isn't in viem's bundled chain registry, so it's defined explicitly here via `defineChain` (this is also the single source of truth the frontend imports for its wagmi config, so chain metadata can't drift between backend and frontend).
- `apps/backend/src/config/env.ts` / `rpc.ts` — added a `RPC_URL_BOTCHAIN` env var, wired into the existing chain-ID-to-RPC-URL map the monitoring engine already used for Ethereum/Base.
- `apps/frontend/providers/wagmi.provider.ts` — added BOT Chain to the RainbowKit/wagmi `chains` array (mainnet only).
- `apps/frontend/utils/explorer.ts` — BOT Chain transaction hashes now link to BOTScan, same pattern as Etherscan/Basescan.
- `apps/frontend/next.config.mjs` — added `@nodara/blockchain` to `transpilePackages` (new frontend dependency, needed to import the shared chain definitions).

### What did NOT need to change

- **Database** — `Wallet` already has `chainId Int` with `@@unique([address, chainId])`; the same address on BOT Chain creates a distinct wallet row automatically. No migration required.
- **Monitoring engine, triggers, conditions** — already resolve chain config per-wallet at evaluation time (`getPublicClient({ chainId, rpcUrl: resolveRpcUrl(chainId) })`); BOT Chain workflows are evaluated by the exact same sweep loop, no separate code path.
- **KeeperHub client** — `SimulationRequest`/`SubmissionRequest` already carry `chainId: number` as a plain field; the client forwards whatever chain ID it's given rather than validating against a hardcoded list.

### Known blockers / external dependencies

- **RPC connectivity has not been tested from this development environment.** `rpc.botchain.ai` needs to be reached from wherever you actually run the backend — confirm with a plain `curl` or `cast block-number --rpc-url https://rpc.botchain.ai` before trusting the monitoring engine against it.
- **Whether the real KeeperHub API supports BOT Chain (677) is unconfirmed.** Nodara's client will happily send a `chainId: 677` request — whether KeeperHub's backend accepts and executes it is entirely up to KeeperHub's own configuration. Confirm with KeeperHub directly before relying on this for BOT Chain executions. (The bundled `apps/keeperhub-mock` local mock has no chain restriction and will "succeed" against any chain ID — it validates the request shape, not real chain support.)

### Smart contract — NodaraExecutionRegistry

A minimal, non-custodial, permissionless contract that gives Nodara's off-chain execution audit trail an optional on-chain anchor on BOT Chain. Full rationale, security review, and deployment/verification instructions live in [`contracts/README.md`](contracts/README.md) — summary:

- Non-payable, no owner, no admin functions, not upgradeable, never holds funds
- Write-once per `executionId` (mirrors Nodara's own off-chain idempotency guarantee)
- **Deployed & Verified on BOT Chain Mainnet**: `0x4e72B76B8f082104b7598C0D3efdc8355FaD87B0` ([BOTScan](https://scan.botchain.ai/address/0x4e72B76B8f082104b7598C0D3efdc8355FaD87B0))

### Creating a BOT Chain workflow

No workflow-builder changes were needed — connect a wallet on BOT Chain (once added to your wallet extension), sign in via SIWE on that network, and the existing builder creates a wallet-scoped workflow exactly as it does for Ethereum/Base. See [Getting started](#getting-started) for the general flow.
