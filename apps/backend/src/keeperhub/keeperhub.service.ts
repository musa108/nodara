...
async function track(keeperHubJobId: string, chainId: number = 677): Promise<TrackingResult> {
  if (isDirectMode) {
    if (keeperHubJobId.startsWith("0x")) {
      try {
        const publicClient = getPublicClient({ chainId, rpcUrl: resolveRpcUrl(chainId) });
